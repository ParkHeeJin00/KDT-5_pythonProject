...
def Whatismylunch():
    ...
    # myLunch가 라면이라면 noodelChoice()로
    if myLunch == '라면':
        noodleChoice()
    # myLunch가 다른 음식이라면 추천 반환
    else:
        print('\n    =================추천 결과=================')
...
# 특별 - 라면 데이터
noodle = ['오징어짬뽕', '진짬뽕', '진순 ㅋㅋ', '삼양라면', '안성탕면', '도시락', '육개장', '김치라면', '멸치칼국수', '참깨라면',
          '열라면', '틈새라면', '신라면', '안성탕면', '진라면 매운맛', '너구리', '생생우동', '사리곰탕']
noodleB = ['팔도비빔면', '불닭볶음면', '짜파게티', '간짬뽕', '라면볶이', '치즈볶이', '콕콕 스파게티', '짜빠구리']


# -------------------------------------------
# 함수 기능 : 랜덤 돌려서 라면 떴을 때, 라면 취향 물어보고 랜덤으로 선택해주는 함수
# 함수 이름 : noodleChoice()
# 매개 변수 : 없음
# 함수 결과 : myLunchNoodle
# --------------------------------------------
def noodleChoice():
    print('''            ===============[이스터에그]=================
            =========띠링띠링 라면에 당첨되셨어요!!========
                         ...
                         다시 한번 골라보세요!
            ================라면메뉴판==================
            1. 국물 있는 라면             2. 국물 없는 라면
            ==========================================
    ''')
    while True:
        myChoice = input('            땡기시는 라면의 번호를 입력해주세용!')
        if myChoice == '1':
            myLunchNoodle = noodle[random.randint(0, len(noodle) - 1)]
            print(f'            국물 있는 라면 중에서 {myLunchNoodle} 추천드려용~!')
            if myLunchNoodle == '진순ㅋㅋ':
                print(f'            {myLunchNoodle} 당첨!... 꼭.. 드세요..!')
            break
        elif myChoice == '2':
            myLunchNoodle = noodleB[random.randint(0, len(noodleB) - 1)]
            print(f'            국물 없는 라면 중에서 {myLunchNoodle} 추천드려용~!')
            break
        else:
            print('            메뉴판의 번호 중에서 골라주세요.')
    return myLunchNoodle
...
def NoneMenu():
    ...
    if myLunch == '라면':
        noodleChoice()
        myLunchNoodle = noodleChoice()
        print(f'''\n    =================== 엥 ===================
    죄송하지만 그런 메뉴는 없어용 ~ 아무거나 드세용! 
    =================랜덤 추천=================
    오늘 점심으로 {myLunchNoodle} 추천드려용~!
    =========================================
